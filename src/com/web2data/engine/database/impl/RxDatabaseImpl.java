package com.web2data.engine.database.impl;
...
public class RxDatabaseImpl implements RxDatabase {
	...
	public RxDatabaseImpl(ConnectionInfo arg) {
		//super();
		this.connectionInfo = arg;
		//this.connection = getDbConnection( arg );
	}
	
	
	//
	public static RxDatabaseImpl openDatabaseForTask( RxTask task ) {
		
		// 根据 Task 中的 app,scenario,stage,stageVersion, 获取该App对应的Database
		
		ConnectionInfo temp = new ConnectionInfo();
		temp.setIp("106.2.1.39");
		temp.setPort(3306);
		temp.setSchema("shuiwu");
		temp.setUsername("root");
		temp.setPassword("HXlEVWBWg0zd5fXbrRuPA");
		temp.setDatabaseType("MYSQL");
		
		RxDatabaseImpl result = new RxDatabaseImpl(temp);
	...
	private void getDatabaseConnection() throws RxCrawlerException {
		
		//Connection result = null;
		try {
			this.connection = ConnectionManager.openConnection( this.connectionInfo );
//			this.connection = ConnectionManager.getConnection( this.connectionInfo );
	...
	public int insert(String sql, Object[] params) throws RxCrawlerException {
		...
		String dbType = connectionInfo.getDatabaseType();
		QueryRunner runner = new QueryRunner();
		try {
			if (dbType.equalsIgnoreCase("ORACLE")) {
				//新的ORACLE QueryRunner 用法
				recordSeq = runner.update(this.connection, sql, params);
			} else {
				//MySQL还用以前的方法
				recordSeq = runner.insert(this.connection, sql, new ScalarHandler<Long>(), params);
			}
		} catch (SQLException e) {
	...
	public int[] batchInsert(String sql, Object[][] params) throws RxCrawlerException {
		...
		//Connection connection = getDbConnection(this.connectionInfo);
		
		int[] cnt = null;

		QueryRunner runner = new QueryRunner();
		try {
			cnt = runner.batch(this.connection, sql, params);
		} catch (SQLException e) {
			throw new RxCrawlerException(e.getMessage());
		} finally {
	...
	public int delete(String sql, Object[] params) throws RxCrawlerException {
		...
		//Connection connection = getDbConnection(this.connectionInfo);
		try {
			cnt = upd(this.connection, sql, params);
		} catch (Exception e) {
			throw new RxCrawlerException(e.getMessage());
		} finally {
	...
	public int update(String sql, Object[] params) throws RxCrawlerException {
		...
		int cnt = -1;
		try {
			upd( this.connection, sql, params );
		} catch (Exception e) {
			throw new RxCrawlerException(e.getMessage());
		} finally {
	...
	public <T> List<T> query(String sql, Class<T> clazz, Object[] params) throws RxCrawlerException {
		...
		try {
			...
			if (dbType.equalsIgnoreCase("ORACLE")) {
				list = (List<T>) runner.query(this.connection, sql, params, new BeanListHandler<T>(clazz));
			} else {
				result = runner.query(this.connection, sql, new MapListHandler(), params);
